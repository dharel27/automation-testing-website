...
router.post('/seed/users', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const userData of sampleUsers) {
            ...
            const user = new User(userData.username, userData.email, hashedPassword, userData.role, userData.profile);
            yield user.save();
            users.push({
                id: user.id,
...
router.post('/seed/products', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const productData of sampleProducts) {
            const product = new Product(productData.name, productData.description, productData.price, productData.category, productData.inStock, productData.imageUrl, productData.tags);
            yield product.save();
            products.push({
                id: product.id,
                name: product.name,
 ...
 */
router.post('/seed/all', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
 ...
 */
router.post('/seed/large-dataset', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (let i = 1; i <= actualCount; i++) {
            ...
            const product = new Product(`Performance Test Product ${i}`, `This is a generated product for performance testing. Product number ${i} with various attributes for filtering and searching.`, Math.round((Math.random() * 1000 + 10) * 100) / 100, // Random price between $10-$1010
            category, Math.random() > 0.2, // 80% chance of being in stock
            null, ['performance', 'test', 'generated', category.toLowerCase(), priority]);
            // Add custom properties for performance testing
            product.priority = priority;
            product.value = Math.floor(Math.random() * 1000);
            product.active = Math.random() > 0.1; // 90% chance of being active
            yield product.save();
            products.push({
                id: product.id,
                name: product.name,
...
router.get('/status', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const productCount = yield db.get('SELECT COUNT(*) as count FROM products');
        const sessionCount = yield db.get('SELECT COUNT(*) as count FROM sessions');
        const fileCount = yield db.get('SELECT COUNT(*) as count FROM file_records');
        res.json({
            success: true,
            data: {
...
router.post('/create-test-user', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { username = 'automation-user', email = 'automation@test.com', password = 'test123', role = 'user', profile = {
