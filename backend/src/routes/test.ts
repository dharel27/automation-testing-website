...
router.get(
  ...
  catchAsync(async (req: Request, res: Response) => {
    ...
    if (isNaN(delay) || delay < 0) {
      throw new AppError(
        'Invalid delay value. Must be a positive number.',
        400,
        'INVALID_DELAY'
      );
    }
    ...
    if (delay > 30000) {
      throw new AppError(
        'Delay too long. Maximum allowed is 30 seconds.',
        400,
        'DELAY_TOO_LONG'
      );
    }
...
router.get(
  ...
  catchAsync(async (req: Request, res: Response) => {
    ...
    switch (errorCode) {
      case 400:
        throw new AppError(
          'Bad Request - This is a simulated 400 error',
          400,
          'BAD_REQUEST'
        );
      case 401:
        throw new AppError(
          'Unauthorized - This is a simulated 401 error',
          401,
          'UNAUTHORIZED'
        );
      case 403:
        throw new AppError(
          'Forbidden - This is a simulated 403 error',
          403,
          'FORBIDDEN'
        );
      case 404:
        throw new AppError(
          'Not Found - This is a simulated 404 error',
          404,
          'NOT_FOUND'
        );
      case 409:
        throw new AppError(
          'Conflict - This is a simulated 409 error',
          409,
          'CONFLICT'
        );
      case 422:
        throw new AppError(
          'Unprocessable Entity - This is a simulated 422 error',
          422,
          'UNPROCESSABLE_ENTITY'
        );
      case 429:
        throw new AppError(
          'Too Many Requests - This is a simulated 429 error',
          429,
          'TOO_MANY_REQUESTS'
        );
      case 500:
        throw new AppError(
          'Internal Server Error - This is a simulated 500 error',
          500,
          'INTERNAL_SERVER_ERROR'
        );
      case 502:
        throw new AppError(
          'Bad Gateway - This is a simulated 502 error',
          502,
          'BAD_GATEWAY'
        );
      case 503:
        throw new AppError(
          'Service Unavailable - This is a simulated 503 error',
          503,
          'SERVICE_UNAVAILABLE'
        );
      case 504:
        throw new AppError(
          'Gateway Timeout - This is a simulated 504 error',
          504,
          'GATEWAY_TIMEOUT'
        );
      default:
        throw new AppError(
          `Unsupported error code: ${errorCode}. Supported codes: 400, 401, 403, 404, 409, 422, 429, 500, 502, 503, 504`,
          400,
...
router.get(
  ...
  catchAsync(async (req: Request, res: Response) => {
    ...
    if (count > 10000) {
      throw new AppError(
        'Dataset too large. Maximum allowed is 10,000 items.',
        400,
        'DATASET_TOO_LARGE'
      );
    }
...
router.get(
  '/memory-intensive',
  catchAsync(async (req: Request, res: Response) => {
    const size = parseInt(req.query.size as string) || 1000000; // 1MB default

    if (size > 10000000) {
      // 10MB limit
      throw new AppError(
        'Memory allocation too large. Maximum allowed is 10MB.',
        400,
...
router.get(
  ...
  catchAsync(async (req: Request, res: Response) => {
    const failureRate = req.query.rate
      ? parseFloat(req.query.rate as string)
    ...
    logger.info(
      `Random failure endpoint called with ${failureRate * 100}% failure rate`
    );
...
router.get(
  '/console-error',
  catchAsync(async (req: Request, res: Response) => {
    const errorType = (req.query.type as string) || 'error';
    ...
    switch (errorType) {
      ...
      default:
        throw new AppError(
          `Unsupported error type: ${errorType}. Supported types: error, warn, info, debug`,
          400,
