...
describe('Session Manager', () => {
  ...
  describe('Token Expiration Times', () => {
    it('should set correct expiration times', () => {
      ...
      const accessTokenExpectedExpiry = new Date(
        beforeGeneration.getTime() + 15 * 60 * 1000
      );
      expect(tokenPair.accessTokenExpiresAt.getTime()).toBeGreaterThanOrEqual(
        accessTokenExpectedExpiry.getTime() - 1000 // Allow 1 second tolerance
      );
      expect(tokenPair.accessTokenExpiresAt.getTime()).toBeLessThanOrEqual(
        new Date(afterGeneration.getTime() + 15 * 60 * 1000).getTime()
      );

      // Refresh token should expire in ~7 days
      const refreshTokenExpectedExpiry = new Date(
        beforeGeneration.getTime() + 7 * 24 * 60 * 60 * 1000
      );
      expect(tokenPair.refreshTokenExpiresAt.getTime()).toBeGreaterThanOrEqual(
        refreshTokenExpectedExpiry.getTime() - 1000 // Allow 1 second tolerance
      );
      expect(tokenPair.refreshTokenExpiresAt.getTime()).toBeLessThanOrEqual(
        new Date(afterGeneration.getTime() + 7 * 24 * 60 * 60 * 1000).getTime()
      );
    });
  });
