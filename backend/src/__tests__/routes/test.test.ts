...
describe('Test Routes', () => {
  ...
  describe('GET /api/test/delay/:ms', () => {
    it('should simulate delay successfully', async () => {
      const startTime = Date.now();
      const response = await request(app).get('/api/test/delay/100');
    ...
    it('should reject invalid delay values', async () => {
      const response = await request(app).get('/api/test/delay/invalid');
    ...
    it('should reject delays that are too long', async () => {
      const response = await request(app).get('/api/test/delay/31000');
  ...
  describe('GET /api/test/error/:code', () => {
    ...
    testCases.forEach(({ code, expectedCode }) => {
      it(`should simulate ${code} error correctly`, async () => {
        const response = await request(app).get(`/api/test/error/${code}`);
    ...
    it('should reject unsupported error codes', async () => {
      const response = await request(app).get('/api/test/error/999');
  ...
  describe('POST /api/test/echo', () => {
    it('should echo request data correctly', async () => {
      const testData = { message: 'Hello, World!' };
      const response = await request(app).post('/api/test/echo').send(testData);
    ...
    it('should handle empty request body', async () => {
      const response = await request(app).post('/api/test/echo');
  ...
  describe('GET /api/test/large-dataset', () => {
    it('should generate default dataset', async () => {
      const response = await request(app).get('/api/test/large-dataset');
    ...
    it('should generate custom sized dataset', async () => {
      const response = await request(app).get(
        '/api/test/large-dataset?count=50'
      );
    ...
    it('should reject datasets that are too large', async () => {
      const response = await request(app).get(
        '/api/test/large-dataset?count=20000'
      );
    ...
    it('should handle delay parameter', async () => {
      const startTime = Date.now();
      const response = await request(app).get(
        '/api/test/large-dataset?count=10&delay=100'
      );
      const endTime = Date.now();
      const actualDelay = endTime - startTime;
  ...
  describe('GET /api/test/memory-intensive', () => {
    it('should handle default memory allocation', async () => {
      const response = await request(app).get('/api/test/memory-intensive');
    ...
    it('should handle custom memory allocation', async () => {
      const response = await request(app).get(
        '/api/test/memory-intensive?size=5000'
      );
    ...
    it('should reject memory allocation that is too large', async () => {
      const response = await request(app).get(
        '/api/test/memory-intensive?size=20000000'
      );
  ...
  describe('GET /api/test/random-failure', () => {
    it('should handle 0% failure rate (always succeed)', async () => {
      const response = await request(app).get(
        '/api/test/random-failure?rate=0'
      );
    ...
    it('should handle 100% failure rate (always fail)', async () => {
      const response = await request(app).get(
        '/api/test/random-failure?rate=1'
      );
    ...
    it('should reject invalid failure rates', async () => {
      const response = await request(app).get(
        '/api/test/random-failure?rate=1.5'
      );
    ...
    it('should reject negative failure rates', async () => {
      const response = await request(app).get(
        '/api/test/random-failure?rate=-0.1'
      );
  ...
  describe('GET /api/test/console-error', () => {
    ...
    testCases.forEach((type) => {
      it(`should generate console ${type} successfully`, async () => {
        const response = await request(app).get(
          `/api/test/console-error?type=${type}`
        );
        ...
        expect(response.body.data.message).toContain(
          `Console ${type} generated`
        );
      });
    });

    it('should handle default error type', async () => {
      const response = await request(app).get('/api/test/console-error');
    ...
    it('should reject unsupported error types', async () => {
      const response = await request(app).get(
        '/api/test/console-error?type=invalid'
      );
